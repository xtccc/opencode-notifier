...
export function loadConfig(): NotifierConfig {
  ...
  try {
    ...
    const globalNotification =
      userConfig.notification ?? DEFAULT_CONFIG.notification;
    ...
    return {
      ...
      pauseOnTyping:
        typeof userConfig.pauseOnTyping === "boolean"
          ? userConfig.pauseOnTyping
          : DEFAULT_CONFIG.pauseOnTyping,
      ...
      command: {
        ...
        path:
          typeof userCommand.path === "string"
            ? userCommand.path
      ...
      events: {
        permission: parseEventConfig(
          userConfig.events?.permission ?? userConfig.permission,
          defaultWithGlobal,
        ),
        complete: parseEventConfig(
          userConfig.events?.complete ?? userConfig.complete,
          defaultWithGlobal,
        ),
        subagent_complete: parseEventConfig(
          userConfig.events?.subagent_complete ?? userConfig.subagent_complete,
          { sound: false, notification: false },
        ),
        error: parseEventConfig(
          userConfig.events?.error ?? userConfig.error,
          defaultWithGlobal,
        ),
        question: parseEventConfig(
          userConfig.events?.question ?? userConfig.question,
          defaultWithGlobal,
        ),
      },
      messages: {
      ...
      sounds: {
        permission:
          userConfig.sounds?.permission ?? DEFAULT_CONFIG.sounds.permission,
        complete: userConfig.sounds?.complete ?? DEFAULT_CONFIG.sounds.complete,
        subagent_complete:
          userConfig.sounds?.subagent_complete ??
          DEFAULT_CONFIG.sounds.subagent_complete,
...
export function isEventSoundEnabled(
