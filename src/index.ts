...
export const NotifierPlugin: Plugin = async ({ client, directory }) => {
  ...
  log(
    `Plugin initialized: suppressWhenFocused=${config.suppressWhenFocused}, focusWindowSeconds=${config.focusWindowSeconds}`,
  );
    ...
    "tool.execute.before": async (input) => {
      if (input.tool === "question") {
        ...
        await handleEventWithElapsedTime(
          ...
          "question",
          projectName,
          { type: "tool.execute.before", properties: { tool: "question" } },
        );
      }
    },
    "tui.input.changed": async (input: { sessionID: string; text: string }) => {
      if (config.pauseOnTyping) {
        log(
          `Hook: tui.input.changed -> user is typing, cancelling notifications`,
        );
        cancelAllPendingNotifications();
      }
    },
